...
namespace snk.framework.nuget
{
    namespace patch
    {
        ...
        {
            private ISnkJsonParser _jsonParser { get; }
                = Snk.Get<ISnkJsonParser>();

            private IEqualityComparer<SnkSourceInfo> _comparer { get; }
                = Snk.Get<IEqualityComparer<SnkSourceInfo>>();

            private ISnkCodeGenerator _codeGenerator { get; }
                = Snk.Get<ISnkCodeGenerator>();
            ...
            public void Build(List<ISnkFileFinder> finderList)
            {
                ...
                {
                    var list = SnkPatch.GenerateSourceInfoList(resVersion.ToString(), finder, ref keyPathMapping);
                    if (list == null)
                ...
                //生成差异列表
                var addList = currSourceInfoList.Except(lastSourceInfoList, this._comparer).ToList();
                var delList = lastSourceInfoList.Except(currSourceInfoList, this._comparer).Select(a => a.key).ToList();
