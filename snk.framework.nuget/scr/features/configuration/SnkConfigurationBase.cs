...
namespace SnkFramework.NuGet.Features
{
    namespace Configuration
    {
        public abstract class SnkConfigurationBase : ISnkConfiguration
        {
            ...
            private List<ISnkTypeConverter> converters = new List<ISnkTypeConverter>();

            public SnkConfigurationBase() : this(null)
            {
            }

            public SnkConfigurationBase(ISnkTypeConverter[] converters)
            {
            ...
            public virtual void AddTypeConverter(ISnkTypeConverter converter)
            {
                this.converters.Insert(0, converter);
            }

            public virtual ISnkConfiguration Subset(string prefix)
            {
                if (string.IsNullOrEmpty(prefix))
                    throw new ArgumentException("the prefix is null or empty", "prefix");

                return new SnkSubsetConfiguration(this, prefix);
            }
