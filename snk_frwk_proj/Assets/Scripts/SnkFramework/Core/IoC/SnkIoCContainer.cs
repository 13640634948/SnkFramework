        ...
        : ISnkIoCProvider
    {
        ...
        public object Resolve(Type type)
        {
            lock (_lockObject)
            {
                ...
                {
                    throw new SnkIoCResolveException("Failed to resolve type {0}", type.FullName);
                }
                return resolved;
            }
        }
        ...
        public void RegisterType(Type t, Func<object> constructor)
        {
            var resolver = new FuncConstructingResolver(() =>
            {
                ...
                if (ret != null && !t.IsInstanceOfType(ret))
                    throw new SnkIoCResolveException("Constructor failed to return a compatibly object for type {0}", t.FullName);
        ...
        public virtual object IoCConstruct(Type type, params object[] arguments)
        {
            ...
            {
                throw new SnkIoCResolveException($"Failed to find constructor for type { type.FullName } with arguments: { arguments.Select(x => x.GetType().Name + ", ") }");
            }
        ...
        public virtual object IoCConstruct(Type type, IDictionary<string, object> arguments)
        {
            ...
            if (selectedConstructor == null)
            {
                throw new SnkIoCResolveException("Failed to find constructor for type {0}", type.FullName);
            }
        ...
        protected virtual object IoCConstruct(Type type, ConstructorInfo constructor, object[] arguments)
        {
            ...
            {
                throw new SnkIoCResolveException(invocation, "Failed to construct {0}", type.Name);
            }
        ...
        private bool InternalTryResolve(Type type, IResolver resolver, out object resolved)
        {
            ...
            if (detectingCircular)
            {
                ...
                catch (ArgumentException)
                {
                    ...
                    SnkLogHost.Default?.Log(LogLevel.Error, "IoC circular reference detected - cannot currently resolve {typeName}", type.Name);
                    resolved = type.CreateDefault();
                    return false;
                }
            }
        ...
        private bool TryResolveParameter(Type type, ParameterInfo parameterInfo, out object parameterValue)
        {
            if (!TryResolve(parameterInfo.ParameterType, out parameterValue))
            {
                ...
                {
                    throw new SnkIoCResolveException(
